...
`page`: 页码，默认为1
`pagesize`: 每页结果数，默认为20
`search_type`: 搜索类型，默认为video

**接口地址 :** `/search`

**调用例子 :** `/search?keyword=音乐`

### B站获取视频详情

说明 : 调用此接口, 传入B站视频BV号，可以获取视频详情
...
**调用例子 :** `/video/detail?bvid=BV1GJ411x7h7`

### B站获取视频播放地址

说明 : 调用此接口, 传入B站视频BV号和cid，可以获取视频播放地址
...
**调用例子 :** `/playurl?bvid=BV1GJ411x7h7&cid=12345678`
